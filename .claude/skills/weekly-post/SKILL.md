...
> ## 운영자의 글과 한마디는 건드리지 않습니다
>
> `categories: ["Hands-on"]` 인 글은 운영자가 직접 쓴 글입니다. 읽기만 하고 고치지 않습니다.
> 어떤 글의 front matter 에도 `note:` 를 **쓰지 않습니다.** `note` 는 "yjworks 한마디" 박스로 나가는
> 운영자 전용 칸이고, 자동으로 채우면 그 박스가 거짓말이 됩니다. 이미 있는 `note` 는 그대로 둡니다.
...
1. `git fetch origin main && git checkout main && git pull origin main`
   ...
   ls content/posts/$TODAY-*.ko.md 2>/dev/null | grep -v -- '-handson-'   # 운영자 글은 세지 않는다
...
조사 중에 이미 발행된 자동 글(Hands-on 제외)의 수치가 틀린 것을 발견하면 고쳐도 된다.
...
- **소제목 검사(글만):** 아래가 아무것도 찍지 않아야 한다. 찍히면 5-1 ②에 따라 소제목을 바꾼다.
  ...
  ```
- 두 파일 모두 front matter 에 `note:` 가 없다(운영자 전용).
