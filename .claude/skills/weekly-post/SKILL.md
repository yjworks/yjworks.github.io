...
- 분량(글): **한국어 1500~2500자**(공백 포함, front matter·표 제외), **영어 700~1200단어**(표 제외).
  한국어는 존댓말.

### `hw` / `embedded` 구조

도입 2~3문장 → `## {제품명}` 상세(스펙 표 + 바로 아래 `출처:` 줄) → 경쟁 제품 비교 표(+ `출처:` 줄) →
장단점과 어떤 사용자에게 맞는지 → 국내 출시 여부 → `## 총평` / `## Verdict`
...
- 갱신할 때 **기존 행을 지우지 않는다.** 단종이 확인되면 모델명 뒤에 "(단종)"을, 후속이 나오면 "(이전 세대)"를 붙인다.
