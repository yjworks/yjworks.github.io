...
제목은 밋밋한 설명문이 아니라 **한 줄로 읽히는 문장**이어야 한다.
...
세로로 읽으면 같은 문장 기계가 찍어낸 것이 보인다. 실제로 14편 중 10편이 이 꼴이 된 적이 있다.
**리듬은 수단이고, 목적은 이 글이 무엇을 말하는지 한 줄로 드러내는 것이다.**
...
- 25자 안팎. 콜론 뒤에 제품명을 나열하는 식은 쓰지 않는다.
- **제품·프로젝트 이름을 하나 이상 넣는다.** 검색 유입이 여기서 나온다. 가능하면 앞쪽에.
- 영어 제목도 같은 원칙으로 따로 만든다. 한국어 제목의 번역이 아니며, 유형도 달라도 된다.
...
**유형을 바꾸려고 없는 결론을 만들지 않는다.** 판정형을 쓰려면 본문에 판정 근거가 있어야 하고,
없으면 그냥 대구를 한 번 더 쓰는 편이 낫다. 형식이 사실보다 앞설 수는 없다.
...
**카드에 제목을 넣지 않는다.** 제목은 목록과 글 상단에 이미 크게 나온다. 카드에까지 넣으면
같은 문장이 두 번 보이고, 카드 전부가 '제목 슬라이드' 한 틀로 읽힌다.
카드가 보여줄 것은 이 블로그가 실제로 파는 것 — **출처로 확인된 수치**다.
...
- **`accent`는 주제에 맞춰 고른다.** 전부 같은 색이면 목록에서 한 틀로 찍어낸 티가 난다.
  ...
  주제 태그가 둘이면 글의 중심이 되는 쪽을 쓴다. ko/en 두 spec 모두 같은 값을 넣는다.
